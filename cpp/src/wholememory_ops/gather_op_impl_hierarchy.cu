  ...
  int gather_sms)
{
  ...
  // exchange node count
  nvtxRangePush("cross gather exchange indices");
  ...
  nvtxRangePop();
  // local gather
  nvtxRangePush("cross gather local gather");
  ...
  nvtxRangePop();
  // exchange embeddings
  nvtxRangePush("cross gather exchange embeddings");
  ...
  int gather_sms)
{
  ...
  try {
    ...
    WM_CUDA_CHECK(cudaStreamSynchronize(stream));
    // printf("sort unique: before=%ld after=%ld repeat-ratio=%.3f\n",
    // recv_bucket_indices_desc.size, cross_gather_indices_desc.size,
    //   (recv_bucket_indices_desc.size-cross_gather_indices_desc.size)
    //   * 1.0f/recv_bucket_indices_desc.size);
    // cross gather
    ...
    // int64_t max_cnt = host_recv_id_count[0], min_cnt = host_recv_id_count[0];
    // for(int i=0;i<host_recv_id_count.size();i++){
    //   max_cnt = max(max_cnt, host_recv_id_count[i]);
    //   min_cnt = min(min_cnt, host_recv_id_count[i]);
    // }
    // printf("exchange max=%ld, min=%ld, ratio=%.2f\n", max_cnt, min_cnt, max_cnt*1.0f/min_cnt);
